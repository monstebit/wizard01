    ...
    out float4 Outline)
{
    ...
    // OutlineColor = depthDifference < 0 ? 0 : (depthEdge > 0.0 ? (DepthEdgeStrength * depthEdge) : (NormalEdgeStrength * normalEdge));
    if (depthDifference < 0)
    {
        Outline = float4(0.0, 0.0, 0.0, 0.0); // Невидимый цвет для отрицательного разницы глубин
    }
    else
    {
        if (depthEdge > 0.0)
        {
            Outline = float4(0.0, 0.0, DepthEdgeStrength * depthEdge, 1.0); // Синий цвет для depthEdge
        }
        else
        {
            Outline = float4(NormalEdgeStrength * normalEdge, 0.0, 0.0, 1.0); // Красный цвет для normalEdge
        }
    }
}
